...
from src.helper.show_feat_coeff import show_feat_coeff
...
class TestShowFeatCoeff(unittest.TestCase):
    ...
    def test_show_feat_coeff_returns_dataframe(self):
        print("Running unit tests for show_feat_coeff.py")

        # Test for correct return type
        result = show_feat_coeff(self.pipe_obj, self.model_name, self.preprocessor)
        self.assertIsInstance(
            result, pd.DataFrame, "show_feat_coeff should return a pandas DataFrame"
        )
    ...
    def test_show_feat_coeff_output(self):
        ...
        output_df = pd.DataFrame(
            data=np.array([0.424879, -0.151969, -0.424879, -1.151919]).T,
            index=["feat1_1.0", "feat3", "feat1_0.0", "feat2"],
    ...
    def test_show_feat_coeff_preprocessor_in_pipeline(self):
        ...
        check_condition = any(
            isinstance(step[1], ColumnTransformer) for step in self.pipe_obj.steps
        )
        self.assertTrue(
            check_condition, "preprocessor must be present in pipeline steps"
        )
